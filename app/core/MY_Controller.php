...
class MY_Controller extends MX_Controller {
    ...
    public function get_total_kanim() {
        $this->load->model('Tbl_helpdesk_imigration_branchs');
        $this->load->driver('cache', array('adapter' => 'apc', 'backup' => 'file'));
        $obj_cache = base64_encode('get_total_kanim');
        if (!$total_rows = $this->cache->get($obj_cache)) {
            $total_rows = $this->Tbl_helpdesk_imigration_branchs->find('count');
            // Save into the cache for 10 minutes
            $this->cache->save($obj_cache, $total_rows, 43200);
        }
    ...
    public function get_office_code_from_ticket($code = null, $opt = 'all') {
        if ($code != null) {
            ...
            if ($opt == 'id') {
                $this->load->model('Tbl_helpdesk_imigration_branchs');
                $r = $this->Tbl_helpdesk_imigration_branchs->find('first', array('conditions' => array('code' => $code[3])));
                return $r['id'];
            } else {
                return $code[3];
            }
