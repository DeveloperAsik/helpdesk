        ...
        <?php if (isset($_menu['nodes']) && !empty($_menu['nodes'])): ?>
            <?php foreach ($_menu['nodes'] AS $key => $values): ?>
                <li class="nav-item<?php echo ($values['is_open'] == 1) ? ' open' : ''; ?>">
                    <a data-path="<?php echo ($values['menu_path'] != '#') ? $values['menu_path'] : '#'; ?>" href="<?php echo $path = ($values['menu_path'] == '#' || $values['menu_path'] == '') ? 'javascript:;' : base_backend_url($values['menu_path']); ?>" class="nav-link <?php echo $path = ($values['menu_path'] == '#' || $values['menu_path'] == '') ? 'nav-toggle' : ''; ?>">
                        ...
                        <ul class="sub-menu"<?php echo ($values['is_open'] == 1) ? ' style="display: block"' : ''; ?>>
                            <?php foreach ($values['nodes'] AS $k => $val): ?>
                                <li class="nav-item<?php echo ($val['is_open'] == 1) ? ' open' : ''; ?>">
                                    <a data-path="<?php echo ($val['menu_path'] != '#') ? $val['menu_path'] : '#'; ?>" href="<?php echo $path = ($val['menu_path'] == '#' || $val['menu_path'] == '') ? 'javascript:;' : base_backend_url($val['menu_path']); ?>" class="nav-link <?php echo $path = ($val['menu_path'] == '#' || $val['menu_path'] == '') ? 'nav-toggle' : ''; ?>">
                                        ...
                                        <ul class="sub-menu"<?php echo ($val['is_open'] == 1) ? ' style="display: block"' : ''; ?>>
                                            <?php foreach ($val['nodes'] AS $k => $v): ?>
                                                <li class="nav-item<?php echo ($v['is_open'] == 1) ? ' open' : ''; ?>">
                                                    <a data-path="<?php echo ($v['menu_path'] != '#') ? $v['menu_path'] : '#'; ?>" href="<?php echo $path = ($v['menu_path'] == '#' || $v['menu_path'] == '') ? 'javascript:;' : base_backend_url($v['menu_path']); ?>" class="nav-link <?php echo $path = ($v['menu_path'] == '#' || $v['menu_path'] == '') ? 'nav-toggle' : ''; ?>">
