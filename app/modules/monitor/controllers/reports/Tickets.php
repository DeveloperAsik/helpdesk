...
class Tickets extends MY_Controller {
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            if (isset($post['param1']) && !empty($post['param1'])) {
                ...
                $act = ' LEFT JOIN (SELECT * FROM tbl_helpdesk_activities d1 WHERE d1.solving_time_stop IN (SELECT max(solving_time_stop) FROM tbl_helpdesk_activities d2 WHERE d2.is_active = 1)) AS d ON d.ticket_id = a.id';
            ...
            $fields = 'a.*, b.status_id, c.name ticket_status, e.name AS priority_name, f.name AS category_name, p.first_name, i.fine_result, i.response_time, i.solving_time, o.close_message, o.response_time_start, o.response_time_stop, o.solving_time_start, o.solving_time_stop, j.job_list, j.message, k.name office_name, k.phone_number, k.address office_address, m.first_name AS created_by, n.ticket_id AS ticket, g.name AS job_category_name' . $f;
            $joins = '
            ...
            if (isset($res) && !empty($res) && $res != null) {
                ...
                if (isset($res) && !empty($res)) {
                    ...
                    foreach ($res as $d) {
                        ...
                        if ($d['solving_time_stop'] == null || $d['solving_time_stop'] == "0000-00-00 00:00:00") {
                            $solving_date = "-";
                            $solving_time = "-";
                        } else {
                        ...
                        $data['office'] = $d['office_name']; //optional
    ...
    public function gen_to_pdf() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $f = ',l.response_message';
            $act = ' LEFT JOIN (SELECT * FROM tbl_helpdesk_activities d1 WHERE d1.solving_time_stop IN (SELECT max(solving_time_stop) FROM tbl_helpdesk_activities d2 WHERE d2.is_active = 1)) AS d ON d.ticket_id = a.id';
            $fields = 'a.*, b.status_id, c.name ticket_status, e.name AS priority_name, f.name AS category_name, p.first_name, i.fine_result, i.response_time, i.solving_time, o.close_message, o.response_time_start, o.response_time_stop, o.solving_time_start, o.solving_time_stop, j.job_list, j.message, k.name office_name, k.phone_number, k.address office_address, m.first_name AS created_by, n.ticket_id AS ticket, d.solving_time_stop, d.solving_time_start, g.name AS job_category_name' . $f;
            $joins = '
            LEFT JOIN tbl_helpdesk_ticket_transactions b ON b.ticket_id = a.id 
            LEFT JOIN tbl_helpdesk_ticket_status c ON c.id = b.status_id 
           ' . $act . '
            ...
            if (isset($res) && !empty($res)) {
                ...
                $txt = '<table class="table" style="width:100%;">                         
                            <tr>
                                ...
                                <td style="width:70%;text-align:center; font-size:12">Laporan tiket helpdesk '.date("Y").'</td>
                            ...
                            <tr>
                                <td style="text-align:center; font-size:8">' . $res[0]['office_address'] . '</td>
                            </tr>
                        </table>';
