...
class Monitoring extends MY_Controller{
    ...
    public function index() {
        redirect(base_url('support/report/monitoring/view/'));
    }
    ...
    public function get_total_ticket_per_month() {
        ...
        foreach ($ticket_date AS $key => $val) {
            ...
            $ticket_list = $this->tbl_helpdesk_ticket_handlers->query("SELECT COUNT(ticket_id) AS total FROM tbl_helpdesk_ticket_handlers WHERE is_active = 1 AND create_date LIKE '{$dt}%' AND created_by = {$user}");
    ...
    public function get_total_ticket_per_month_by_status(){
        ...
        $user = (int) base64_decode($this->auth_config->user_id);
        $year = date('Y');
        $ticket_status = $this->Tbl_helpdesk_ticket_transactions->query("SELECT a.id, a.name, COUNT(b.status_id) total FROM tbl_helpdesk_ticket_status a LEFT JOIN tbl_helpdesk_ticket_transactions b ON b.status_id = a.id 
            LEFT JOIN tbl_helpdesk_ticket_handlers c ON c.ticket_id = b.ticket_id 
            WHERE c.created_by = {$user} OR a.id = 1
            GROUP BY a.id ");
        // debug($this->db->last_query());
        $arr_chart = array();
    ...
    public function get_total_ticket_progress_per_month(){
        ...
        $ticket_handlers = $this->Tbl_helpdesk_ticket_handlers->query("SELECT id, YEAR(create_date) AS year, MONTH(create_date) AS month, MONTHNAME(create_date) AS 'month_name' FROM tbl_helpdesk_ticket_handlers WHERE create_date LIKE '{$year}%' GROUP BY month_name ORDER BY month ASC");
        // debug($this->db->last_query());
        $arr_chart = array();
        foreach ($ticket_handlers AS $key => $val) {
            // $ticket_id = $val['ticket_id'];
            $dt = $val['year'] . '-' . str_pad($val['month'], 2, '0', STR_PAD_LEFT);
            // $ticket_list = $this->Tbl_helpdesk_ticket_transactions->query("SELECT COUNT(a.ticket_id) AS total  FROM tbl_helpdesk_ticket_transactions a
            //     LEFT JOIN tbl_helpdesk_ticket_handlers b ON b.ticket_id = a.ticket_id WHERE a.ticket_id = {$ticket_id} AND a.status_id = 2 AND b.user_id = {$user_id}");
            $ticket_list = $this->Tbl_helpdesk_ticket_transactions->query("SELECT COUNT(a.ticket_id) AS total  FROM tbl_helpdesk_ticket_transactions a
                LEFT JOIN tbl_helpdesk_ticket_handlers b ON b.ticket_id = a.ticket_id WHERE a.status_id = 2 AND b.user_id = {$user_id} AND b.create_date LIKE '{$dt}%'");
            $arr_chart[] = array(
                'status' =>$val['month_name'],
                'total' => (int) $ticket_list[0]['total']
            );
            // debug($arr_chart);
        }
