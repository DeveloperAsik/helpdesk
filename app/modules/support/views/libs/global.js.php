<script>
    ...
    var fnCheckResponseBtn = function (id) { var return_val = false; var url_post = base_url + 'support/ticket/check_status_ticket'; var formdata = { id: id }; $.ajax({ url: url_post, method: "POST", data: formdata, success: function (response) { if (response == 'true') { return true; } else { return false; } } }); };
    var fnGetTimtikUser = function () { var url_post = base_url + 'support/user/get_user/1'; $.ajax({ url: url_post, method: "POST", success: function (response) { $('.timtik').html(response); } }); return false; };
    var fnGetVendorUser = function () { var url_post = base_url + 'support/user/get_user/2'; $.ajax({ url: url_post, method: "POST", success: function (response) { $('.support').html(response); } }); return false; };
    var fnTicketAuth = function (status) { switch (status) { case 'request-to-close': $('#sbmt_message').attr('disabled', 'disabled'); $('#req_to_close').attr('disabled', 'disabled'); $('#req_to_close').attr('title', 'This ticket already request to be close...'); break; case 'close': $('#re_open').show(); $('#sbmt_message').attr('disabled', 'disabled'); $('#req_to_close').hide(); $('.caption-helper').css('color:red'); break; case 'progress': $('#re_open').hide(); $('#mark_as_solve').attr('disabled', 'disabled'); break; case 'open': $('#re_open').hide(); break; } };
    ...
    var fnInterval = function () {
        setInterval(function () {
            ...
            if (open === true) {
                ...
                var url_post = base_url + 'support/ticket/check_ticket_timeout/';
                $.ajax({
                    url: url_post,
                    method: "POST",
                    data: formdata,
    ...
    var fnCheckTicketOpen = function (id) {
        var formdata = {id: (id)};
        $.ajax({url: base_url + 'support/ticket/check_ticket_open/', method: "POST", data: formdata, success: function (response) {
                if (response === 'false') {
    ...
    var fnCloseTicket = function () {
        var id = $('input[name="ticket_id"]').val();
        if (id) {
            var formdata = {id: Base64.encode(id)};
            $.ajax({url: base_url + 'support/ticket/set_close/', method: "POST", data: formdata, success: function (response) {
                    return false;
                }, error: function (response) {
                    return false;
                }});
    ...
    var sendFile = function (file, that, opt) {
        var data = new FormData();
        data.append('file', file);
        $.ajax({
            data: data,
            type: 'post',
            url: base_url + 'support/tracking/insert_image_summernote/' + opt.ticket_id + '/' + opt.code,
            cache: false,
            contentType: false,
            processData: false,
    ...
    var fnSetOpenTicket = function (formdata) {
        $.ajax({
            url: base_url + 'support/ticket/set_open/',
            method: "POST",
            data: formdata,
            success: function (response) {
    ...
    var fnCloseTicket = function (formdata) {
        ...
        $.ajax({
            url: base_url + 'support/ticket/set_close/',
            method: "POST",
            data: formdata,
            success: function (response) {
                console.log(response);
    ...
    var fnGetDetail = function (id) {
        ...
        $.ajax({
            url: base_url + 'support/ticket/get_ticket_detail/',
            method: "POST",
            data: formdata,
            beforeSend: function () {
                // Statement
                ...
                if (row.ticket_status == 'close' || row.ticket_status == 'progress' || row.ticket_status == 'transfer' || row.ticket_status == 'open') {
                    if(row.ticket_status == 'open'){
                        ...
                        if (row.history_chat) {
                            ...
                            var username = history_chat[i]['username'];
                            if (history_chat[i]['created_by'] == 0) {
                                username = 'By System';
                            }
    ...
    var fnCheckTicketOpenStatus = function (id) {
        ...
        $.ajax({
            url: base_url + 'support/ticket/check_status_ticket/',
            method: "POST",
            data: formdata,
            success: function (data) {
                if (data == 'false') {
    ...
    var fnCheckTicketTransfer = function(id){
        $.post( base_url + 'support/ticket/check_total_transfer_ticket/' + id, function( data ) {
            if(data == 'false'){
                $('.modal').modal('hide');
    ...
    var fnGetTransferTicket = function (id) {
        ...
        $.ajax({
            url: base_url + 'support/ticket/get_data/',
            method: "POST",
            data: formdata,
            beforeSend: function () {
                // Statement
    ...
    var fnGetResponseTicket = function (id) {
        ...
        $.ajax({
            url: base_url + 'support/ticket/get_ticket_detail/response',
            method: "POST",
            data: formdata,
            beforeSend: function () {
                // Statement
    ...
    var fnGetCloseTicket = function (ticket_id) {
        ...
        $.ajax({
            url: base_url + 'support/tracking/get_job_desc',
            method: "POST",
            data: formdata,
            beforeSend: function () {
                // Statement
            ...
            success: function (data) {
                $('input[name="ticket_id_close_ticket"]').val((ticket_id));
                $('#job_list_').html(data);
            },
            error: function (data) {
                console.log(data);
    ...
    var fnGetCategory = function () {
        $.ajax({
            url: base_url + 'support/ticket/get_category/',
    ...
    var fnGetJob = function (id, v) {
        $.ajax({
            url: base_url + 'support/ticket/get_category/' + id + '/job',
            method: "POST", //First change type to method here
            success: function (response) {
                if (v == 2) {
    ...
    var GlobalAjax = function () {
        return {
            init: function () {
                ...
                $(document).scroll(function () {
                    if (($(document).height() - $(window).height() - $(document).scrollTop()) < 300) {
                        if (($(document).height() - $(window).height() - $(document).scrollTop()) == 0) {
                            $(".actions .actDatatables").css("position", '');
                            $('.actions .actDatatables').on('scroll').css('z-index', '');
                        } else {
                            $('.actions .actDatatables').on('scroll').css('position', 'fixed');
                            $('.actions .actDatatables').on('scroll').css('right', '0px');
                            $('.actions .actDatatables').on('scroll').css('z-index', '9999');
                        }
                    }
                });
                //---------------------------------------------------------------------------------------------//
                ...
                //---------------------------------------------------------------------------------------------//
                //js handler from action a href modal hied start here
                //---------------------------------------------------------------------------------------------//
                ...
                //---------------------------------------------------------------------------------------------//
                //js handler from action a href modal hide start here
                //---------------------------------------------------------------------------------------------//
                ...
                $('#frmResponse').on('submit', function (e) {
                    e.preventDefault();
                    var uri = base_url + 'support/ticket/response_ticket';
                        ...
                        success: function (response) {
                             ...
                             var dialog = bootbox.dialog({
                                 ...
                                 buttons: {
                                     success: {
                                         ...
                                         className: "green",
                                         callback: function () {
                                             window.location.href = base_url + 'support/tracking/view/' + Base64.encode(ticket_code);
                                         }
                                     },
                                     danger: {
                                         label: "Ticket List",
                                         className: "grey",
                                         callback: function () {
                                             window.location.href = base_url + 'support/ticket/view/progress';
                                         }
                                     }
                                 }
                             });
                             dialog.init(function () {
                                 ...
                                 var interval = setInterval(function () {
                                     ...
                                     dialog.find('.bootbox-body').html('This will redirect to ticket tracking after <b style="color:#000; font-size:12px;">' + max + '</b>');
                                     if (counter == 3) {
                                         // Display a login box
                                         // $(window).unbind('beforeunload');
                                         // window.onbeforeunload = null;
                                         window.open(base_url + 'support/tracking/view/' + Base64.encode(ticket_code), '_blank');
                                         $('.modal').modal('hide');
                                         window.location.href = base_url + 'support/ticket/view/progress';
                                     }
                                     ;
                ...
                //user start here
                //---------------------------------------------------------------------------------------------//                
                $('#opn_agreement_trans_tickt').on('click', function (e) {
                    e.preventDefault();
                    var uri = base_url + 'support/ticket/agreement_to_take_over_ticket';
                    ...
                    var ticket = Base64.encode($('input[name="ticket_id_modal_transfer_open"]').val());//Base64.encode
                ...
                $('#frmTransfer').on('submit', function (e) {
                    var uri = base_url + 'support/user/transfer_ticket';
                    var formdata = {
                    ...
                    bootbox.confirm("Are you sure?", function (result) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                window.location = base_url + 'support/ticket/view/progress';
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
                                App.stopPageLoading();
                ...
                $('#frmClose').on('submit', function (e) {
                    var uri = base_url + 'support/tracking/close_ticket_request';
                    arr = [];
                    ...
                    bootbox.confirm("Are you sure?", function (result) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                App.stopPageLoading();
                                setTimeout(function () {
                                    window.location = base_url + 'support/ticket/view/close';
                                }, '3000');
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
                                App.stopPageLoading();
