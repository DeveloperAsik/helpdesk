 ...
 * @author SuperUser
 */
class Immigration_branch extends MY_Controller {

    //put your code here

    public function __construct() {
        parent::__construct();
        $this->load->model(array('Tbl_helpdesk_imigration_branchs'));
    }
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_helpdesk_imigration_branchs';
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            if (isset($search) && !empty($search)) {
                $cond['or_like'] = $cond_count['or_like'] = array('a.name' => $search);
            }
            $total_rows = $this->Tbl_helpdesk_imigration_branchs->find('count', $cond_count);
            $res = $this->Tbl_helpdesk_imigration_branchs->find('all', $cond);
            $arr = array();
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_helpdesk_imigration_branchs->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_helpdesk_imigration_branchs->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_imigration_branchs->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_imigration_branchs->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_helpdesk_imigration_branchs->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_helpdesk_imigration_branchs->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
