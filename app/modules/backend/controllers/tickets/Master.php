...
class Master extends MY_Controller {
    ...
    public function get_list($key = null) {
        ...
        if (isset($post) && !empty($post)) {
            $this->load->model(array('Tbl_helpdesk_imigration_branchs', 'Tbl_helpdesk_ticket_priorities', 'Tbl_helpdesk_ticket_requests', 'Tbl_helpdesk_ticket_reopen_logs'));
    ...
    public function transfer_ticket() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $this->load->model(array('Tbl_helpdesk_tickets', 'Tbl_helpdesk_imigration_branchs', 'Tbl_helpdesk_ticket_transfers', 'Tbl_user_groups', 'Tbl_helpdesk_ticket_handlers', 'Tbl_helpdesk_vendor_users', 'Tbl_helpdesk_ticket_transactions', 'Tbl_helpdesk_ticket_chats', 'Tbl_helpdesk_activities', 'Tbl_helpdesk_ticket_files'));
            $user_to = 0;
            ...
            if ($res) {
                ...
                $imi_branchs = $this->Tbl_helpdesk_imigration_branchs->find('first', array('conditions' => array('code' => $get_office_code)));
