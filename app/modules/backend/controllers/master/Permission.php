...
class Permission extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(array('Tbl_permissions'));
    }

    public function index() {
        redirect(base_backend_url('master/permission/view/'));
    }

    public function view() {
        $data['title_for_layout'] = 'welcome';
        $data['view-header-title'] = 'View Permission List';
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_permissions';
            if (isset($search) && !empty($search)) {
                $cond['like'] = $cond_count['like'] = array('a.name', $search);
            }
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            $total_rows = $this->Tbl_permissions->find('count', $cond_count);
            $config = array(
                'base_url' => base_backend_url('master/permission/get_list/'),
                'total_rows' => $total_rows,
                'per_page' => $length,
            );
            $this->pagination->initialize($config);
            $res = $this->Tbl_permissions->find('all', $cond);
            $arr = array();
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_permissions->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_permissions->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_permissions->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_permissions->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_permissions->remove($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_permissions->remove($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
    ...
    public function delete() {
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                $arr_res = 1;
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_permissions->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_permissions->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
