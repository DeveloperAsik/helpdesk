...
class Group extends MY_Controller {
    ...
    public function index() {
        redirect(base_backend_url('master/group/view/'));
    }

    public function view() {
        $data['title_for_layout'] = $this->lang->line('global_title_for_layout_group');
        $data['view-header-title'] = $this->lang->line('global_header_title_group');
        $js_files = array(
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_groups';
            if (isset($search) && !empty($search)) {
                $cond['like'] = $cond_count['like'] = array('a.name', $search);
            }
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            $total_rows = $this->Tbl_groups->find('count', $cond_count);
            $res = $this->Tbl_groups->find('all', $cond);
            $arr = array();
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_groups->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_groups->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_groups->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_groups->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_groups->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
