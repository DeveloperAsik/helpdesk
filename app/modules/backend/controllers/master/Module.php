...
class Module extends MY_Controller {
    ...
    public function index() {
        redirect(base_backend_url('master/module/view/'));
    }

    public function view() {
        $data['title_for_layout'] = $this->lang->line('global_title_for_layout_module');
        $data['view-header-title'] = $this->lang->line('global_header_title_module');
        $js_files = array(
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_modules';
            if (isset($search) && !empty($search)) {
                $cond['like'] = $cond_count['like'] = array('a.name', $search);
            }
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            $total_rows = $this->Tbl_modules->find('count', $cond_count);
            $res = $this->Tbl_modules->find('all', $cond);
            $arr = array();
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_modules->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_modules->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_modules->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_modules->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_modules->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_modules->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
