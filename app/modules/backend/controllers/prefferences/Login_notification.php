...
/**
 * Description of Login_notification
 *
 * @author SuperUser
 */
class Login_notification extends MY_Controller {

    //put your code here

    public function __construct() {
        parent::__construct();
        $this->load->model(array('Tbl_helpdesk_login_notifications'));
    }

    public function index() {
        redirect(base_backend_url('prefferences/login_notification/view/'));
    }

    public function view() {
        $data['title_for_layout'] = $this->lang->line('global_title_for_layout_login_notif');
        $data['view-header-title'] = $this->lang->line('global_header_title_login_notif');
        $var = array(
            array(
                'keyword' => 'login_notif',
                'value' => isset($this->config->login_notification) ? $this->config->login_notification : 1
            )
        );
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_helpdesk_login_notifications';
            if (isset($search) && !empty($search)) {
                $cond['like'] = $cond_count['like'] = array('a.name', $search);
            }
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            $total_rows = $this->Tbl_helpdesk_login_notifications->find('count', $cond_count);
            $res = $this->Tbl_helpdesk_login_notifications->find('all', $cond);
            $arr = array();
            if (isset($res) && !empty($res)) {
                ...
                foreach ($res as $d) {
                    ...
                    $data['content_summary'] = $d['content_summary']; //optional
                    $data['content_full'] = $d['content_full']; //optional	
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_helpdesk_login_notifications->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_helpdesk_login_notifications->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_login_notifications->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_login_notifications->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_helpdesk_login_notifications->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_helpdesk_login_notifications->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
