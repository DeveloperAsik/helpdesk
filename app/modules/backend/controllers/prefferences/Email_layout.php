...
/**
 * Description of Email Layout
 *
 * @author SuperUser
 */
class Email_layout extends MY_Controller {

    //put your code here

    public function __construct() {
        parent::__construct();
        $this->load->model(array('Tbl_email_layout'));
    }

    public function index() {
        redirect(base_backend_url('prefferences/email_layout/view/'));
    }

    public function view() {
        $data['title_for_layout'] = $this->lang->line('global_title_for_layout_email_layout');
        $data['view-header-title'] = $this->lang->line('global_header_title_email_layout');
        $js_files = array(
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_email_layout';
            if (isset($search) && !empty($search)) {
                $cond['like'] = $cond_count['like'] = array('a.keyword', $search);
            }
            $cond['fields'] = array('a.*');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            $total_rows = $this->Tbl_email_layout->find('count', $cond_count);
            $res = $this->Tbl_email_layout->find('all', $cond);
            $arr = array();
            if (isset($res) && !empty($res)) {
                ...
                foreach ($res as $d) {
                    ...
                    $data['num'] = $i;
                    $data['keyword'] = $d['keyword']; //optional	
                    $data['value_eng'] = $d['value_eng']; //optional	
                    $data['value_ind'] = $d['value_ind']; //optional	
    ...
    public function get_data() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $res = $this->Tbl_email_layout->find('first', array(
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_email_layout->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_email_layout->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_email_layout->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_email_layout->delete($val);
                }
                if ($arr_res == true) {
                    echo 'success';
                } else {
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_email_layout->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
