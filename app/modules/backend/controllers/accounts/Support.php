...
class Support extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(array('Tbl_helpdesk_supports'));
    }

    public function index() {
        redirect(base_backend_url('accounts/support/view/'));
    }

    public function view() {
        $data['title_for_layout'] = $this->lang->line('global_title_for_layout_support');
        $data['view-header-title'] = $this->lang->line('global_header_title_support');
        $js_files = array(
    ...
    public function download_sample($filename, $type) {
        ...
        $this->oreno_generate_file->init($filename, 'support', $type);
    }

    public function import_file() {
        if (isset($_FILES['import_file_support']) && !empty($_FILES['import_file_support'])) {
            $path = $_FILES['import_file_support']['tmp_name'];
            ...
            if ($data) {
                ...
                $this->load->model(array('Tbl_users', 'Tbl_user_groups', 'Tbl_helpdesk_supports', 'Tbl_helpdesk_support_users', 'Tbl_helpdesk_supports'));
                if (isset($arr) && !empty($arr)) {
                    foreach ($arr AS $k => $val) {
                        $office = $this->Tbl_helpdesk_supports->find('first', array('conditions' => array('code' => $val['office_id'])));
                        $pass = array();
                        ...
                        $this->Tbl_helpdesk_support_users->insert($arr_insert);
                    }
                    echo 'success';
                }
            }
    ...
    public function get_list() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_helpdesk_supports';
            $cond['fields'] = array('a.*');
            ...
            $total_rows = $this->Tbl_helpdesk_supports->find('count', $cond_count);
            $res = $this->Tbl_helpdesk_supports->find('all', $cond);
            $arr = array();
            if (isset($res) && !empty($res)) {
                ...
                foreach ($res as $d) {
                    ...
                    $action_status = '<div class="form-support">
    ...
    public function get_support_user_list() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            ...
            $cond_count = array();
            $cond['table'] = $cond_count['table'] = 'Tbl_helpdesk_supports';
            $cond['fields'] = array('a.*', 'b.code support_code', 'b.name support_name', 'c.username', 'c.first_name', 'c.last_name', 'c.email', 'e.name group_name');
            $cond['limit'] = array('perpage' => $length, 'offset' => $start);
            ...
            $total_rows = $this->Tbl_helpdesk_support_users->find('count', $cond_count);
            $res = $this->Tbl_helpdesk_support_users->find('all', $cond);
            $arr = array();
            if (isset($res) && !empty($res)) {
                $i = $start + 1;
                foreach ($res as $d) {
                    ...
                    $data['support_code'] = $d['support_code']; //optional
                    $data['support_name'] = $d['support_name']; //optional
                    $data['email'] = $d['email']; //optional
    ...
    public function get_data() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_supports->find('first', array('conditions' => array('a.id' => base64_decode($post['vndr_id']))));
            if (isset($res) && !empty($res)) {
    ...
    public function get_data_support_user() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $this->load->model('Tbl_helpdesk_support_users');
            $res = $this->Tbl_helpdesk_support_users->find('first', array(
                'fields' => array('a.*', 'b.code support_code', 'b.name support_name', 'c.id user_id', 'c.username', 'c.first_name', 'c.last_name', 'c.email', 'e.name group_name'),
    ...
    public function insert() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $result = $this->Tbl_helpdesk_supports->insert($arr_insert);
            if ($result == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_supports->update($arr, base64_decode($post['id']));
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status($id_ = null) {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_support_users->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
        ...
        if (isset($post) && !empty($post)) {
            if (is_array($post['id'])) {
                ...
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_helpdesk_supports->delete($val);
                ...
                $res = $this->Tbl_helpdesk_supports->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
    ...
    public function delete_user() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            if (is_array($post['id'])) {
                $arr_res = 1;
                foreach ($post['id'] AS $key => $val) {
                    $arr_res = $this->Tbl_helpdesk_support_users->delete($val);
                    $this->tbl_users->delete();
                }
                if ($arr_res == true) {
                    echo 'success';
                ...
                $id = base64_decode($post['id']);
                $res = $this->Tbl_helpdesk_support_users->delete($id);
                if ($res == true) {
                    echo 'success';
                } else {
                    echo 'failed';
    ...
    public function insert_user() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $category_id = 0;
            $contract_id = 1;
            $this->load->model(array('Tbl_users', 'Tbl_user_groups', 'Tbl_helpdesk_supports', 'Tbl_helpdesk_support_users'));
            $pass = array();
            ...
            $res = $this->Tbl_helpdesk_support_users->insert($arr_insert);
            if ($res) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_user() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $this->load->model(array('Tbl_users', 'Tbl_helpdesk_support_users'));
            $arr_user = array(
                'username' => $post['username'],
                'first_name' => $post['first_name'],
                'last_name' => $post['last_name'],
            ...
            if ($res) {
                ...
                $this->Tbl_helpdesk_support_users->update_by($arr_insert, base64_decode($post['id']), 'user_id');
                echo 'success';
            }
        } else {
            echo 'failed';
    ...
    public function get_supports() {
        $res = $this->Tbl_helpdesk_supports->find('all', array('conditions' => array('is_active' => 1), 'order' => array('key' => 'name', 'type' => 'ASC')));
        if (isset($res) && !empty($res)) {
    ...
    public function assign_category_user() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_support_users->update($arr_insert, base64_decode($post['id']));
            if ($res) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_status_support($id_ = null) {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $id = base64_decode($id_);
            $status = 0;
            ...
            $res = $this->Tbl_helpdesk_supports->update($arr, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function insert_support() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_supports->insert($arr_insert);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
    ...
    public function update_support() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            $res = $this->Tbl_helpdesk_supports->update($arr_update, $id);
            if ($res == true) {
                echo 'success';
            } else {
                echo 'failed';
