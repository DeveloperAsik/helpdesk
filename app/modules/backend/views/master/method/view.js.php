        ...
        return {
            ...
            init: function () {
                var table = $('#datatable_ajax').DataTable({
                    ...
                    "ajax": {
                        url: base_backend_url + 'master/method/get_list/',
                        type: 'POST'
                    },
                    "columns": [
                        {"data": "rowcheck"},
                ...
                $('#datatable_ajax').on('switchChange.bootstrapSwitch', 'input[name="status"]', function (event, state) {
                    bootbox.confirm("Are you sure?", function (result) {
                        ...
                        $.ajax({
                            url: base_backend_url + 'master/method/update_status/',
                ...
                $('a.btn').on('click', function () {
                    ...
                    switch (action) {
                        case 'add':
                            $('.modal-title').html('Insert New Method');
                            break;

                        case 'edit':
                            $('.modal-title').html('Update Exist Method');
                            ...
                            if (status_ == 0) {
                                ...
                                $.ajax({
                                    url: base_backend_url + 'master/method/get_data/',
                                    method: "POST", //First change type to method here
                                    data: formdata,
                                    success: function (response) {
                        ...
                        case 'delete':
                            bootbox.confirm("Are you sure to delete this id?", function (result) {
                                ...
                                var uri = base_backend_url + 'master/method/delete/';
                                if (count > 1) {
                                    id = [];
                ...
                $("#add_edit").submit(function () {
                    ...
                    var uri = base_backend_url + 'master/method/insert/';
                    ...
                    if (id) {
                        uri = base_backend_url + 'master/method/update/';
                        txt = 'update group';
                        formdata = {
                            id: Base64.encode(id),
                            name: $('input[name="name"]').val(),
