        ...
        return {
            ...
            init: function () {
                var table = $('#datatable_ajax').DataTable({
                    ...
                    "ajax": {
                        url: base_backend_url + 'prefferences/config/get_list/',
                        type: 'POST'
                    },
                    "columns": [
                        {"data": "rowcheck"},
                ...
                $('#datatable_ajax').on('switchChange.bootstrapSwitch', 'input[name="status"]', function (event, state) {
                    bootbox.confirm("Are you sure?", function (result) {
                        ...
                        $.ajax({
                            url: base_backend_url + 'prefferences/config/update_status/',
                ...
                $('a.btn').on('click', function () {
                    ...
                    switch (action) {
                        case 'add':
                            $('.modal-title').html('Insert New User');
                            break;
                        ...
                        case 'edit':
                            ...
                            if (status_ == 0) {
                                ...
                                $.ajax({
                                    url: base_backend_url + 'prefferences/config/get_data/',
                                    method: "POST", //First change type to method here
                                    data: formdata,
                                    success: function (response) {
                        ...
                        case 'delete':
                            bootbox.confirm("Are you sure to delete this id?", function (result) {
                                ...
                                var uri = base_backend_url + 'prefferences/config/delete/';
                                id = [];
                ...
                $("#add_edit").submit(function () {
                    ...
                    var uri = base_backend_url + 'prefferences/config/insert/';
                    var txt = 'add new config';
                    var formdata = {
                    ...
                    if (id) {
                        uri = base_backend_url + 'prefferences/config/update/';
                        txt = 'update config';
                        formdata = {
                            id: Base64.encode(id),
                            keyword: $('input[name="keyword"]').val(),
