        ...
        return {
            ...
            init: function () {
                var table = $('#datatable_ajax').DataTable({
                    ...
                    "ajax": {
                        url: base_backend_url + 'tickets/status/get_list/',
                        type: 'POST'
                    },
                    "columns": [
                        {"data": "rowcheck"},
                ...
                $('#datatable_ajax').on('switchChange.bootstrapSwitch', 'input[name="status"]', function (event, state) {
                    ...
                    $.ajax({
                        url: base_backend_url + 'tickets/status/update_status/',
                ...
                $('a.btn').on('click', function () {
                    ...
                    switch (action) {
                        case 'add':
                            $('.modal-title').html('Insert New Status');
                            break;

                        case 'edit':
                            $('.modal-title').html('Update Exist Status');
                            ...
                            if (status_ == 0) {
                                ...
                                $.ajax({
                                    url: base_backend_url + 'tickets/status/get_data/',
                                    method: "POST", //First change type to method here
                                    data: formdata,
                                    success: function (response) {
                        ...
                        case 'delete':
                            bootbox.confirm("Are you sure to delete this id?", function (result) {
                                ...
                                var uri = base_backend_url + 'tickets/status/delete/';
                                id = [];
                ...
                $("#add_edit").submit(function () {
                    ...
                    var uri = base_backend_url + 'tickets/status/insert/';
                    ...
                    if (id) {
                        uri = base_backend_url + 'tickets/status/update/';
                        txt = 'update group';
                        formdata = {
                            id: Base64.encode(id),
                            name: $('input[name="name"]').val(),
