...
class User extends MY_Controller {
    ...
    public function get_user($id = 1) {
        if ($id == 1) {
            ...
            $res = $this->Tbl_helpdesk_vendor_users->find('all', array(
                ...
                'conditions' => array('a.is_active' => 1, 'vendor_id !=' => $this->auth_config->office_id),
                'joins' => array(
                    array(
                        'table' => 'tbl_users b',
    ...
    public function transfer_ticket() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $this->load->model(array('Tbl_helpdesk_tickets', 'Tbl_helpdesk_ticket_files', 'Tbl_helpdesk_ticket_transfers', 'Tbl_user_groups', 'Tbl_helpdesk_ticket_handlers', 'Tbl_helpdesk_vendor_users', 'Tbl_helpdesk_ticket_transactions', 'Tbl_helpdesk_ticket_chats', 'Tbl_helpdesk_activities'));
            $user_to = 0;
            // if ($post['group'] == 2) {
            //     $user_ = $this->Tbl_helpdesk_vendor_users->find('first', array('conditions' => array('id' => $post['vendor'])));
    ...
    public function update() {
        $post = $this->input->post(NULL, TRUE);
        if (isset($post) && !empty($post)) {
            $this->load->model(array('Tbl_users', 'Tbl_user_profiles', 'Tbl_helpdesk_vendor_users'));
